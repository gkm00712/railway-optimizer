...
def get_duration(wagons, pair_name):
    """Calculates strictly the UNLOADING duration (without shunting)."""
...
def format_dt(dt):
    """Safely formats datetime objects, handling NaT/NaN errors."""
    if pd.isnull(dt):
        return "N/A"
...
if uploaded_file is not None:
    ...
    if not all(col in df.columns for col in required_cols):
        ...
        # NOTE: Filter strictly for Loaded trains (Optional - uncomment next 2 lines to enable)
        ...
        # CRITICAL FIX: Drop rows where *effective* arrival time is invalid
        # But keep rows where Exp Arrival might be NaT if Revised Arrival is valid
        ...
        for _, rake in df.iterrows():
            ...
            # --- SCENARIO A: Pair A ---
            dur_unload_A = get_duration(rake['wagon_count'], 'Pair A (T1&T2)')
            ...
            # 2. Ready to Unload (Line Entry + Shunting Time)
            ready_to_unload_A = entry_time_A + shunt_offset_A
            
            # 3. Actual Start (Max of Ready Time OR Tippler Free)
            start_unload_A = max(ready_to_unload_A, pair_state['Pair A (T1&T2)'])
            finish_A = start_unload_A + timedelta(hours=dur_unload_A)
            
            # --- SCENARIO B: Pair B ---
            dur_unload_B = get_duration(rake['wagon_count'], 'Pair B (T3&T4)')
            ...
            start_unload_B = max(ready_to_unload_B, pair_state['Pair B (T3&T4)'])
            finish_B = start_unload_B + timedelta(hours=dur_unload_B)
            ...
            if finish_A <= finish_B:
                ...
                best_dur_display = dur_unload_A + (SHUNT_MINS_A/60.0) 
                ...
                best_dur_display = dur_unload_B + (SHUNT_MINS_B/60.0)
